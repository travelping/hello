...
handle_incoming_message(Context, ExUriURL, Binary) ->
    {ok, _, Listener = #listener{protocol = ProtocolMod, router = Router}} = hello_registry:lookup({listener, ExUriURL}),
    {ok, BinResp} = hello_proto:handle_incoming_message(Context, ProtocolMod, Router, ExUriURL, Binary),
