...
-define(REQ_TRACES(Mod), [{class, hello}, {hello_request, api}, {hello_handler, Mod}]).
-define(BAD_TRACES(Mod), [{class, hello}, {hello_request, error}, {hello_handler, Mod}]).

-define(LOG_REQUEST_async_reply(CallbackModule, HandlerPid, Request, Response),
    lager:info(?REQ_TRACES(CallbackModule), "async reply on ~p. ~s - ~s", 
...
-define(LOG_REQUEST_request(CallbackModule, HandlerPid, Request, Response, Time),
    lager:info(?REQ_TRACES(CallbackModule), "request on ~p. ~s - ~s (~p ms)", 
...
-define(LOG_REQUEST_request_stop(CallbackModule, HandlerPid, Request, Response, Reason, Time),
    lager:info(?REQ_TRACES(CallbackModule), "request on ~p. ~s - ~s (stopped with reason ~p) (~p ms)", 
...
-define(LOG_REQUEST_request_no_reply(CallbackModule, HandlerPid, Request, Time),
    lager:info(?REQ_TRACES(CallbackModule), "request on ~p. ~s - noreply (~p ms)", 
...
-define(LOG_REQUEST_request_stop_no_reply(CallbackModule, HandlerPid, Request, Time),
    lager:info(?REQ_TRACES(CallbackModule), "request on ~p. ~s (stopped with reason normal) (~p ms)", 
               [HandlerPid, hello_log:fmt_request(Request), Time])).

-define(LOG_REQUEST_request_stop_no_reply(CallbackModule, HandlerPid, Request, Reason, Time),
    lager:info(?REQ_TRACES(CallbackModule), "request on ~p. ~s (stopped with reason ~p) (~p ms)", 
...
-define(LOG_REQUEST_bad_request(CallbackModule, HandlerPid, Request, Reason),
    lager:error(?BAD_TRACES(CallbackModule), "bad request on ~p. ~s - ~p", 
